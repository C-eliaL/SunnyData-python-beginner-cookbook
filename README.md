# SunnyData Python Beginner Cookbook
...
- **DAY2：流程控制 + 函式雛形**
  ...
  - `project-personal-finance`：結合條件、迴圈與資料結構的迷你專案
- **References**：與 12 小時課程教案對應，可延伸到後續模組（集合、檔案 I/O、期末專案等）

## 🚀 學生如何在 Colab 使用教材
1. **打開教材列表**  
   - 進入 GitHub 倉庫首頁（`SunnyData-python-beginner-cookbook`）並捲動到 README 的教材清單。  
   - 每個 .ipynb 都會附上一個 `Open in Colab` 徽章按鈕。
2. **點擊 `Open in Colab`**  
   - 按下徽章後，瀏覽器會自動開啟 Google Colab 頁面。  
   - 若第一次使用，請先登入自己的 Google 帳號。
3. **儲存雲端複本並開始操作**  
   - 在 Colab 內點 `檔案 > 在雲端硬碟中儲存複本`，把筆記本複製到個人 Drive。  
   - 依提示選擇 `執行階段 > 變更執行階段類型`（預設 CPU 即可），再依序執行每個程式區塊。
4. **遇到問題的備用作法**  
   - 若網路無法連到 Colab，可按 README 提供的 GitHub 連結，下載 `.ipynb` 後在本機 Jupyter/VS Code 開啟。  
